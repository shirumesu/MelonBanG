...
const dmhyMagnet =
  "magnet:?xt=urn:btih:ABCDEFGHIJKLMNOPQRSTUVWXYZ234567&amp;dn=%E8%91%AC%E9%80%81";
...
describe("SourceService", () => {
  ...
  it("enqueues only an opaque stored candidate and hands bounded torrent bytes to downloads", async () => {
    const create = vi.fn<(input: TorrentInput) => DownloadTaskView>(() => downloadTask());
    ...
    await expect(service.enqueue({ candidateId: "https://evil.example/file.torrent" })).rejects.toThrow(
      "资源已过期，请重新搜索。"
    );
