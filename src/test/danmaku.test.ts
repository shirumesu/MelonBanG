...
describe("ArtPlayer danmaku mapping", () => {
    ...
    expect(
      toArtPlayerDanmuku(
        [
          ...
          { timeSeconds: 100, text: "顶部", mode: "top", color: "#ffd56b" },
          { timeSeconds: 110, text: "底部", mode: "bottom", color: "#ffffff" }
        ],
        100
      )
    ).toEqual([
      { time: 0, text: "顶部", mode: 1, color: "#ffd56b" },
      { time: 10, text: "底部", mode: 2, color: "#ffffff" }
    ]);
  });
  ...
  it("drops invalid or empty comments before they reach the plugin", () => {
    ...
    ).toEqual([{ time: 4, text: "保留并清理空格", mode: 0, color: "#ffffff" }]);
  });
